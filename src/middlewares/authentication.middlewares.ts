...
const credentials = (req: Request, res: Response, next: NextFunction) => {
  // Skip authentication if user already set by devBypass middleware
  if (req.user) {
    console.log("Auth skipped (DEV)");
    return next();
  }

  const authHeader = req.headers.authorization;
  if (!authHeader) {
    return res.status(HttpStatus.BAD_REQUEST).json({ success: false, message: "Missing Authorization Header" });
  }
